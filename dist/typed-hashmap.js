 ...
 */
function getHash(defaultValue, hash, key, map) {
...
function setTree(node, size, map) {
    ...
    return node === getNode(map)
        ? map
        : new HashMap(node, size);
}

function findHash(key) {
    var hash = typeof key === 'number'
        ? key
        : typeof key === 'string'
            ? stringHash(key)
...
function setKeyValue(key, value, map) {
    ...
    var size = { value: getSize(map) };
    var newNode = getNode(map).modify(0, constant(value), hash, key, size);
...
function get(key, map) {
    ...
    return function getValueFromMap(_map) {
...
//# sourceMappingURL=typed-hashmap.js.map
